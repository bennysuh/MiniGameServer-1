...
def BrodCasting():
    ...
    myPrint("broadCast Thread start")
    ...
    while True and flagBrodcastQueue.empty():
        if True:
            ...
            if msgQueue.qsize() > 0:
                ...
                myPrint("brodcast at ",time.ctime())
                print "msgQueue qsize :",msgQueue.qsize()
                myPrint("msgQueue qsize :",msgQueue.qsize())
                for i in range(msgQueue.qsize()):
                    s = msgQueue.get()
                    if s=='' or s.find("$") < 0 :
                        continue
                    print i,"st str :",repr(s)
                    myPrint(i,"st str :",repr(s))
                    tmpStr += s
            mutex.acquire()
            for k in pawnDic.keys():
                if pawnDic[k].IsAlive() == False:
                    ...
                    myPrint('%s is dead, remove from pawnDic'%k)
                    print "current user names :", pawnDic.keys()
                    myPrint("current user names :", pawnDic.keys())
            mutex.release()
            if len(tmpStr) > 0:
                print "BroadCasting thread, sending"
                myPrint("BroadCasting thread, sending")
                mutex.acquire()
                for k in pawnDic.keys():
                    tmpPawn = pawnDic[k]
                    if tmpPawn.IsAlive():
                        print "send msg to user: ",k
                        myPrint("send msg to user: ",k)
                        pawnDic[k].Send(tmpStr)
                mutex.release()
                print "BroadCasting thread, sended"
                myPrint("BroadCasting thread, sended")
            
        time.sleep(0.1)
    print "broadCast Thread end"
    myPrint("broadCast Thread end")
...
while True:
    ...
    myPrint("get connection from client....addr: ",str(addr))
